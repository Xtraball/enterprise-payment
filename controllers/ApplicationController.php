...
class Enterprisepayment_ApplicationController extends Application_Controller_Default
{
    ...
    public function loadstripeformAction()
    {
        ...
        $detailmodel = new Enterprisepayment_Model_Gatewaydetail();
        $gdata = $detailmodel->find(['gid' => $pgid, 'value_id' => $pvalue_id]);
    ...
    public function savestripeformAction()
    {
        ...
        try {
            ...
            if ($values['payment_mode'] == 1) {
                ...
                if (!empty($errors)) {
                    $message = [__("Please enter all stripe credentials after clicking on edit option")];
            ...
            $liveDataArray = ['live_publishkey' => $values['live_publishkey'], 'live_secrete_key' => $values['live_secrete_key']];

            $testDataArray = ['test_publishkey' => $values['test_publishkey'], 'test_secrete_key' => $values['test_secrete_key']];
            ...
            $gdata = $detailmodel
                ->find(['gid' => $values['gid'], 'value_id' => $values['value_id']])
                ->setData($postArray)
                ->save();
    ...
    public function savepaypaldetailAction()
    {
        ...
        try {
            ...
            if ($values['payment_mode'] == 1) {
                ...
                if (!empty($errors)) {
                    $message = [__("Please enter all paypal credentials after clicking on edit option")];
                    foreach ($errors as $error) {
                        $message[] = $error;
                    }
                    $message = join(',', $message);
            ...
            $liveDataArray = ['username' => $values['username'], 'signature' => $values['signature'], 'password' => $values['password']];

            $sandboxDataArray = ['sandboxusername' => $values['sandboxusername'], 'sandboxsignature' => $values['sandboxsignature'], 'sandboxpassword' => $values['sandboxpassword']];
            ...
            $detailmodel = new Enterprisepayment_Model_Gatewaydetail();
            $gdata = $detailmodel->find(['id' => $values['id'], 'value_id' => $values['value_id']])
                ->setData($postArray)->save();
        ...
        try {
            ...
            for ($i = 0; $i <= $total_link_url_fields; $i++) {
                ...
                if (!empty($return_url_link) || !empty($return_state)) {
                    $get_all_array_link_val = ['id' => $id_url_link,
                        'value_id' => $value_id,
            ...
            $setting_data = $setting_model->findAll(['value_id' => $value_id]);
            $get_all_return_value_id = [];
            $get_all_link = [];
            $get_all_state = [];
            ...
            foreach ($get_all_link_val as $key => $link_url_values) {
                if (in_array($link_url_values['return_value_id'], $value_id_in_array)) {
                    ...
                    if ($link_url_values['return_value_id']) {
                        $setting_model = new Enterprisepayment_Model_Setting();
                        $setting_model
                            ->find(['value_id' => $value_id, 'id' => $link_url_values['id']])
                            ->setValueId($value_id)
    ...
    public function settingdataAction()
    {
        ...
        try {
            $setting_model = new Enterprisepayment_Model_Setting();
            $setting_data = $setting_model->find(['value_id' => $values['value_id']])->getData();
            $data = [
                "success" => 1,
                "message" => __("Success"),
                "data" => $setting_data,
    ...
    public function savebanktransferformAction()
    {
        ...
        try {
            ...
            $detailmodel = new Enterprisepayment_Model_Gatewaydetail();
            if ($postArray != null) {
                $gdata = $detailmodel->find(['value_id' => $value_id, 'gid' => $gid])
                    ->setData($postArray)->save();
            }
            $data = [
                'success' => '1',
    ...
    public function addlabelfieldAction()
    {
        try {
            ...
            $getfields = $detailmodel->find(['field_name' => $field_name]);
    ...
    public function loadbanktransferformAction()
    {
        ...
        try {
            ...
            $get_bank_fields = $load_data->findAll(['value_id' => $values['value_id']]);
            $get_form_fields = [];
            foreach ($get_bank_fields as $key => $value) {
                $get_form_fields[] = $value->getData();
            }
            $load_data = new Enterprisepayment_Model_Gatewaydetail();
            $get_bank_data = $load_data->find(['value_id' => $values['value_id'], 'gid' => $values['gid']]);
    ...
    public function savepayulatamAction()
    {
        ...
        try {
            ...
            if ($values['payment_mode'] == 1) {
                ...
                if (!empty($errors)) {
                    $message = [__("Please enter all payulatam credentials after clicking on edit option")];
                    foreach ($errors as $error) {
                        $message[] = $error;
                    }
                    $message = join(',', $message);
            ...
            $liveDataArray = ['live_payulatam_app_id' => $values['live_payulatam_app_id'], 'live_payulatam_public_key' => $values['live_payulatam_public_key'], 'live_payulatam_private_key' => $values['live_payulatam_private_key']];

            $testDataArray = ['test_payulatam_app_id' => $values['test_payulatam_app_id'], 'test_payulatam_public_key' => $values['test_payulatam_public_key'], 'test_payulatam_private_key' => $values['test_payulatam_private_key']];
            ...
            $detailmodel = new Enterprisepayment_Model_Gatewaydetail();
            $gdata = $detailmodel->find(['id' => $values['id']])
                ->setData($postArray)
                ->save();
    ...
    public function saveurllinkAction()
    {
        ...
        try {
            ...
            $return_value_id = end($return_value_id);
            $check_url_available = $model_save_url->find(['value_id' => $value_id, 'return_value_id' => $return_value_id])->getData();
            if (($values['return_link'] != '-1')) {

                if ($values['id']) {
                    $model_save_url
                        ->find(['value_id' => $value_id, 'id' => $values['id']])
                        ->setValueId($value_id)
                    ...
                    $model_save_url
                        ->find(['value_id' => $value_id, 'return_value_id' => $return_value_id])
                        ->setValueId($value_id)
                        ->setReturnLink($return_link)
                        ->setReturnUrl($return_url)
                        ->setReturnValueId($return_value_id)
    ...
    public function loadsettingformAction()
    {
        try {
            ...
            $detailmodel = new Enterprisepayment_Model_Setting();
            $setting_data = $detailmodel->find(['id' => $id, 'value_id' => $value_id])->getdata();
            $getdynamic_data = ['value_id' => $value_id, 'return_link' => $setting_data['return_link'], 'return_url' => $setting_data['return_url'], 'id' => $setting_data['id']];
            $response_data = [
                'success' => true,

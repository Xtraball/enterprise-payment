...
require_once Core_Model_Directory::getBasePathTo("app/local/") . 'modules/Enterprisepayment/controllers/Api/stripe-php/init.php';
...
class Enterprisepayment_Mobile_ViewController extends Application_Controller_Mobile_Default {
	...
	public function paypalpaymentAction() {
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				$paypalCredentials = $detailmodel->find(array('gid' => $data['gid'],'value_id' => $data['value_id']));
				if ($paypalCredentials->getData()) {
					...
					$stripeCredentials = $detailmodel->find(array('value_id' => $data['value_id'],'return_value_id' => $data['return_value_id']));
					...
					$html = array('success' => 1, 'token_url' => $this->__pay_url . '&webview=1', 'gid' => $paypalCredentials->getGid(),'return_url'=> $return_url,'return_state'=> $return_state,'return_value_id' => $return_value_id);

				} else {
					$html = array('success' => 0, 'message' => __("Don't have Paypal Credential!"));
				}

			} catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}
		} else {
			$html = array('error' => 1, 'message' => __('An error occurred during process. Please try again later.'));
		}
	...
	public function transactionAction() {
		if ($this->getRequest()->getRawBody()) {
			try {
				$data = Zend_Json::decode($this->getRequest()->getRawBody());
				$rData = array('token' => $data['token'],'payer' => $data['payer']);
				...
				$detailmodel = new Enterprisepayment_Model_Setting();
				$papCredentials = $detailmodel->find(array('value_id' => $data['value'],'return_value_id' => $data['return_value_id']));
				...
				$html = array('success' => 1, 'return_url' => $return_url, 'return_state' => $return_state, 'return_value_id' => $return_value_id);
			} catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}
		} else {
	...
	public function stripepaymentAction() {
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				$sData = array('id' => $stripe_id,'balance_transaction' => $balance_transaction);
				...
				if($status_response == 1){
					$detailmodel = new Enterprisepayment_Model_Setting();
					$stripeCredentials = $detailmodel->find(array('value_id' => $data['value'],'return_value_id' => $data['return_value_id']));
					$return_url = $stripeCredentials->getReturnLink();
					$return_state = $stripeCredentials->getReturnState();
					$return_value_id = $stripeCredentials->getReturnValueId();
				}
	...
	public function getpublishkeyAction(){
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				$detailmodel = new Enterprisepayment_Model_Gatewaydetail();
				$stripeCredentials = $detailmodel->find(array('gid' => $data['id'],'value_id' => $data['value']));
				if ($stripeCredentials->getData()) {
					...
					$html = array('success' => 0, 'message' => __("Don't have stripe Credential!"));
				}
			}catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}
	...
	public function getbanktransferformAction(){
		if ($this->getRequest()->getRawBody()) {
			try {
				$data = Zend_Json::decode($this->getRequest()->getRawBody());
				$detailmodel = new Enterprisepayment_Model_Gatewaydetail();
				$bank_data = $detailmodel->find(array('gid' => $data['gid'],'value_id' => $data['value_id']));
				if ($bank_data->getData()) {
					...
					foreach ($json_data as $key => $value) {
						...
						$getfields = $bankfieldsmodel->find(array('id' => $key));
						unset ($json_data[$key]);
						$new_key =  $getfields->getFieldName();
						$json_data[$new_key] = $value;
					}
					$bankfieldsmodel = new Enterprisepayment_Model_Banksetting();
					$getfields = $bankfieldsmodel->find(array('id' => $id));
					...
					$html = array('success' => 0, 'message' => __("Don't have bank transfer Credential!"));
				}
			}catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}
	...
	public function postbankresponseAction(){
		if ($this->getRequest()->getRawBody()) {
			...
			try {
				...
				$response = array('token' => $token, 'type' => 'bank_transfer_response');
				...
				$detailmodel = new Enterprisepayment_Model_Setting();
				$bank_response = $detailmodel->find(array('value_id' => $data['value_id'],'return_value_id' => $data['return_value_id']));
				...
				$html = array('success' => 1,'return_url' => $return_url,'return_state' => $return_state,'return_value_id' => $return_value_id, 'token' => $token);
			} catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}
		} else {
	...
	public function postcashresponseAction(){
		if ($this->getRequest()->getRawBody()) {
			try {
				$data = Zend_Json::decode($this->getRequest()->getRawBody());
				$token = $this->generateRandomString();
				$response = array('token' => $token, 'type' => 'cash');
				$jsonResponse = json_encode($response);
				$application_id = $this->getApplication()->getId();
				$transactionmodel = new Enterprisepayment_Model_Transaction();
				/*Amount with proper format*/
	...
	function sendemail($customer_id,$date,$amt,$token,$payment_type){
		...
		$customer_getdata = $customersmodel->find(array('customer_id' => $customer_id))->getData();
		$currency_name = Core_Model_Language::getCurrentCurrency()->getShortName();
		$currency = Core_Model_Language::getCurrentCurrency()->getSymbol();
	...
	public function getpayulatamtokenAction(){
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				$detailmodel = new Enterprisepayment_Model_Gatewaydetail();
				$payu_latam_credentials = $detailmodel->find(array('gid' => $data['id'],'value_id' => $data['value']));
		...
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				if ($payu_latam_credentials->getData()) {
					...
					$myorder_id  	= array('id' => "myorderid".$order_id, );
					$post_for_payment_id  = array(
		...
		if ($this->getRequest()->getRawBody()) {
			try {
				...
				$provider_specific_data = array(
					...
					//////////////////
					$post_form_payment_response  = array(
					...
					$html = array('success' => 0, 'message' => __("Don't have PayU Latam Credential!"));
				}
			}catch (Exception $e) {
				$html = array('success' => 0, 'message' => $e->getMessage());
			}

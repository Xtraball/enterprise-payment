App.controller('EnterprisepaymentController', function ($scope, $state, $stateParams, $location, Enterprisepaymentpro,
                                                        Application, $ionicLoading) {
    ...
    if (Application.is_webview) {
        ...
        if (url.indexOf("?") > -1) {
            ...
            if (second_split.length > 1) {
                ...
                Enterprisepaymentpro.transaction($scope.value_id, amount, customer_id, 1, gid, 1, $scope.return_value_id).success(function (data) {
                    if (data.success == 1) {
                        ...
                        if (data.return_state.length > 0) {
                            ...
                            $location.path(BASE_PATH + '/' + data.return_url).search({
                                transaction_id: Enterprisepaymentpro.payerId,
                ...
                console.log('---- ----- ----- ---->');
                console.log('1');
                $state.go('enterprisepayment_process', {
                    value_id: $scope.value_id,
                    amount: Enterprisepaymentpro.get_data('amount'),
                    return_value_id: $stateParams.value_id,
                    order_id: 456
                }, {reload: true});
            }
        }
    ...
    $scope.openpayment = function (isValid, amount) {
        if (isValid) {
            ...
            $state.go('enterprisepayment_process', {
                value_id: $scope.value_id,
                amount: amount,
                return_value_id: $scope.value_id,
                order_id: 456
            }, {reload: true});
        }
    }
}).controller('EnterprisepaymentProcessController', function ($translate, $location, $ionicLoading, $state, $ocLazyLoad,
                                                              $ionicLoading, $ionicModal, $rootScope, $scope, $stateParams,
                                                              Customer, SB, Dialog, SocialSharing, Modal,
                                                              Enterprisepaymentpro, $ionicPopup) {
    $scope.is_loading = true;
    $scope.value_id = Enterprisepaymentpro.value_id = $stateParams.value_id;
                                ...
                                Enterprisepaymentpro.getstripe(token, $scope.amount, $scope.secretekey, method_gid, $scope.order_id, $scope.return_value_id).success(function (data) {
                                    if (data.success == 1) {
                                        ...
                                        if (data.return_state.length > 0) {
                                            ...
                                            $location.path(BASE_PATH + '/' + $scope.return_url).search({
